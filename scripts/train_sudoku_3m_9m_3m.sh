...
export RUN_DATASET="sudoku-3m-9m"
export RUN_MODEL="sudoku-3m"
...
python main.py \
  ...
  optim.lr=4e-4 \
  training.ema=0.9999 \
  trainer.max_steps=1250000 \
  trainer.val_check_interval=1.0 \
  +trainer.check_val_every_n_epoch=5 \
  callbacks.checkpoint_every_n_steps.every_n_train_steps=25000 \
  trainer.log_every_n_steps=50 \
  +callbacks.phase_split_checkpoint._target_=phase_checkpoint.PhaseSplitCheckpoint \
  +callbacks.phase_split_checkpoint.boundary_step=125000 \
  ...
  'hydra.run.dir=${mdlm_root:}/outputs/sudoku3m9m-mix/${oc.env:RUN_MODEL}/${now:%Y.%m.%d}/${now:%H%M%S}' \
  wandb.project=sudoku-mdlm \
  wandb.group=sudoku-3m9m-v0 \
  wandb.name=${RUN_NAME} \
